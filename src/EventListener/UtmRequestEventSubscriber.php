...
class UtmRequestEventSubscriber implements EventSubscriberInterface
{
    public function __construct(
        ...
        private readonly UtmParameterRepository $utmParametersRepository,
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        // 查找或创建UTM参数实体
        $parameters = $this->utmParametersRepository->findByParams($validatedParamsDto);

        if (null === $parameters) {
            // 创建新的UTM参数实体
            $parameters = new UtmParameter();
            $parameters->setSource($validatedParamsDto->getSource());
            $parameters->setMedium($validatedParamsDto->getMedium());
            $parameters->setCampaign($validatedParamsDto->getCampaign());
            $parameters->setTerm($validatedParamsDto->getTerm());
            $parameters->setContent($validatedParamsDto->getContent());

            // 处理额外的UTM参数
            $additionalParams = $validatedParamsDto->getAdditionalParameters();
            if ([] !== $additionalParams) {
